...
export const getProducts = new Promise((resolve) => {
 ...
 export const getBooks = async (url) => { //async transforma mi funcion en una funcion asincronica
  ...
  export const getBookById = async (url, id) => {
    // async transforma mi función en una función asincrónica
    const respuesta = await fetch(url); // fetch resuelve la API, no tiene un then ni nada, pero la resuelve simplemente, y es guardada en una constante 
    const data = await respuesta.json(); // ahora la const respuesta es pasada por un json() para que sea tangible por JavaScript y alojada en otra constante 
    const book = data.books.find(item => item.isbn13 === id); // utilizamos find para buscar el producto con el ID proporcionado
    return book; // retornamos el producto encontrado (o undefined si no se encuentra)
  };
